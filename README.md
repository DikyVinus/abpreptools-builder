...
### 3. Run the workflow
- Open **Actions**
- Select: `Build Flashable Partition ZIP`
- Click **Run workflow**

Fill inputs:

- `ARCHIVE_URL` → direct link to archive containing `.img` partitions  
- `OUTPUT_NAME` → output zip name (e.g. `flashable`)  
...
Pipeline:
```text
download → extract → collect .img → generate updater-script → build zip

Runtime:

typically 1–5 minutes depending on archive size
```
...
Use before flashing AOSP or custom ROMs when:
...
Recovery-flashable ZIP containing:
